...
#tally the sample sizes and create the label
 length_counts <- species_lengths %>%
  ...
  summarise(n = n()) %>%
   pivot_wider(id_cols = Name, names_from = SITE, values_from = n, values_fill = 0) %>%
   mutate(Label = paste("MPA: N = ", MPA, "\n REF: N = ", REF)) %>%
   select(Name, Label)
 

 #join the labels to the dataframe
 spp_length_plot_dat <- left_join(spp_length_plot_dat, length_counts)
 
 #set x and y coordinates fr the labels
 o
  ...
  geom_histogram(aes(y =..density..), binwidth = 2, alpha=0.5, position = 'identity') +
   ...
   geom_text(aes(x = 30, y = .4, label = Label), inherit.aes = FALSE, size = 2, family = "sans")
        # print(p1)
})
}
...
```{r fig-lengths, fig.cap = paste("Density of",names(plot_list), "by 2 cm fork length bins encountered inside each MPA and outside at reference areas (REF)."), echo=FALSE, results = "asis"}
